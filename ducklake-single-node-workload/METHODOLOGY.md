...
Each profile received one warm-up and five measured complete runs. Runs were serial and used deterministic seed `20260901`. The OS page cache was not flushed.

## Measurement boundaries

Every data notebook used `time.perf_counter_ns()` around work owned by that step. The filesystem-backed runner's timestamps supplied end-to-end duration. These remain separate because notebook launch, orchestration, persistence, and static HTML retention sit outside the internal data timers.
...
- CPython 3.14.6
- Anatini 0.9.0 filesystem-backed runner
- DuckDB 1.5.5
- DuckLake extension `d8a1881e`
- marimo 0.23.16
- DuckDB execution profile: 6 GB memory limit, four threads
