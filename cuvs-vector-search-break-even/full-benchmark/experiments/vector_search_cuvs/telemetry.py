...
class GpuSampler:
    ...
    def stop(self) -> dict[str, Any]:
        ...
        for left, right in zip(self.samples, self.samples[1:]):
            energy += (right["seconds"] - left["seconds"]) * (left["power_watts"] + right["power_watts"]) / 2
        return {
            ...
            "utilization_mean_percent": sum(x["utilization_percent"] for x in self.samples) / len(self.samples),
